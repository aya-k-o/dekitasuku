...
| ログイン | https://ayako-k.com/dekitasuku/login.php |
| 子ども画面 | ログイン後、トップページから子どもを選択 |
| 管理画面 | ログイン後、https://ayako-k.com/dekitasuku/admin_today.php |
...
3. 「タスク管理」でタスクとポイントを設定
4. 子ども画面でタスクを達成するとポイント獲得！
5. 「きょうのにっき」で体調や気持ちを記録
6. 親は「今日の様子」から日記に返信できます
...
| パスワードハッシュ化 | `password_hash()` / `password_verify()` でハッシュ化保存・照合 |
| SQLインジェクション対策 | PDOプリペアドステートメント |
| XSS対策 | `htmlspecialchars()` による出力エスケープ（h関数として共通化） |
| セッション管理 | `session_start()` + `$_SESSION` でログイン状態を保持 |
| セッション固定化攻撃対策 | ログイン成功時に `session_regenerate_id(true)` でセッションID再生成 |
| 認証ガード | 全ページでセッション確認、未ログインはリダイレクト |
| 認可チェック | `user_id` でデータの所有者確認、他の家族のデータにアクセス不可 |
...
### 6. マルチテナント設計
`users`テーブルで家族単位のアカウントを管理。`children`テーブルに`user_id`を持たせることで、複数家族が同じアプリを安全に使用できます。URL直打ちによる他の家族のデータへのアクセスも、認可チェックで防止しています。
...
http://localhost:8080/register.php からアカウントを作成してください。
